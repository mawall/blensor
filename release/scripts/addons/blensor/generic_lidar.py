...
parameters = {"angle_resolution": 0.25,
              "rotation_speed": 40,
              "max_dist": 30,
              "noise_mu": 0.0,
              "noise_sigma": 0.03,
              ...
              "reflectivity_slope": 0.005333333,
              "laser_angles": "0.0",
...
def addProperties(cType):
    global parameters
    cType.generic_angle_resolution = bpy.props.FloatProperty(name="Scanresolution",
                                                             default=parameters["angle_resolution"], min=0.01,
                                                             max=3.141,
                                                             description="How far(angle) two scan lines are apart")
    cType.generic_rotation_speed = bpy.props.FloatProperty(name="Rotation speed", default=parameters["rotation_speed"],
                                                           min=1, max=100, description="Rotation speed in Hertz")
    cType.generic_max_dist = bpy.props.FloatProperty(name="Scan distance", default=parameters["max_dist"], min=0,
                                                     max=1000, description="How far the laser can see")
    cType.generic_noise_mu = bpy.props.FloatProperty(name="Noise mu", default=parameters["noise_mu"],
                                                     description="The center of the gaussian noise")
    cType.generic_noise_sigma = bpy.props.FloatProperty(name="Noise sigma", default=parameters["noise_sigma"],
                                                        description="The sigma of the gaussian noise")
    cType.generic_start_angle = bpy.props.FloatProperty(name="Start angle", default=parameters["start_angle"],
                                                        description="The angle at which the scan is started")
    cType.generic_end_angle = bpy.props.FloatProperty(name="End angle", default=parameters["end_angle"],
                                                      description="The angle at which the scan is stopped")

    cType.generic_ref_dist = bpy.props.FloatProperty(name="Reflectivity Distance",
                                                     default=parameters["reflectivity_distance"],
                                                     description="Objects closer than reflectivity distance are "
                                                                 "independent of their reflectivity")
    cType.generic_ref_limit = bpy.props.FloatProperty(name="Reflectivity Limit",
                                                      default=parameters["reflectivity_limit"],
                                                      description="Minimum reflectivity for objects at the "
                                                                  "reflectivity distance")
    cType.generic_ref_slope = bpy.props.FloatProperty(name="Reflectivity Slope",
                                                      default=parameters["reflectivity_slope"],
                                                      description="Slope of the reflectivity limit curve")
    cType.generic_laser_angles = bpy.props.StringProperty(name="Laser angles", default=parameters["laser_angles"],
                                                          description="Comma separated list of vertical angles")
    cType.generic_advanced_error_model = bpy.props.StringProperty(name="Advanced Error Model",
                                                                  default=parameters["advanced_error_model"],
                                                                  description="List of (distance, mu, sigma) tuples, "
                                                                              "leave empty if standard error model is "
                                                                              "used")
...
# If the laser noise has to be truely randomize, call this function prior to every scan
...
def scan_advanced(scanner_object,
    ...
    for i in range(len(returns)):
        ...
        reusable_vector.xyzw = [norm_vector[0] * vector_length_noise,
                                norm_vector[1] * vector_length_noise,
                                norm_vector[2] * vector_length_noise,
                                1.0]
